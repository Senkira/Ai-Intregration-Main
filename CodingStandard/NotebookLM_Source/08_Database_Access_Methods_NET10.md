...
## 12. การตัดสินใจ — ADO.NET vs Dapper (ผลวิจัยเพิ่มเติม)

> **วันที่ตัดสินใจ:** 4 มีนาคม 2026  
> **ผลลัพธ์:** ✅ **ใช้ Dapper เป็นมาตรฐานหลัก** — ❌ **ไม่ใช้ ADO.NET ดิบ**

### 12.1 Benchmark จริงจาก Published Sources

| Query Type | ADO.NET | Dapper | ส่วนต่าง | แหล่งอ้างอิง |
|---|---|---|---|---|
| Single Row (PK Lookup) | 0.013 ms (13μs) | 0.047 ms (47μs) | ADO เร็วกว่า ~3.6x | exceptionnotfound.net |
| Multi-Row (Filtered List) | 1.03 ms | 1.01 ms | **เท่ากัน** | exceptionnotfound.net |
| Complex JOIN (Multi-table) | 8.84 ms | 7.94 ms | **Dapper เร็วกว่า ~10%** | exceptionnotfound.net |

> ⚠️ Benchmark ใช้ `SqlDataAdapter` ใน ADO.NET — ถ้าใช้ `SqlDataReader` จะเร็วขึ้น แต่ code verbose มากขึ้นด้วย

### 12.2 Dapper Internal Overhead Analysis

| Metric | รายละเอียด |
|---|---|
| **Mapping Mechanism** | IL Emit (`System.Reflection.Emit`) — สร้าง delegate ครั้งเดียว |
| **Cache** | `ConcurrentDictionary<Identity, CacheInfo>` — key = SQL + type + connection |
| **First Call (Cold)** | microseconds–low milliseconds (IL compilation) |
| **Subsequent Calls (Warm)** | **near-zero overhead** — cache hit, delegate invoke ตรง |
| **Overhead vs total query time** | **<1%** — ส่วนใหญ่ถูก dominate ด้วย DB + Network latency |

### 12.3 Memory Allocation

| Metric | ADO.NET (DataReader) | Dapper |
|---|---|---|
| Single Row | ~100–500 bytes | ~1–5 KB |
| 100+ Rows | Similar per-row scale | เพิ่มเล็กน้อยจาก mapper |
| GC Pressure | ต่ำมาก | ต่ำ (ไม่มีนัยสำคัญ) |

> Dapper allocate มากกว่า ADO.NET ~3-10x ต่อ query แต่อยู่ในระดับ **KB ไม่ใช่ MB**

### 12.4 Developer Productivity

| Metric | ADO.NET | Dapper |
|---|---|---|
| Lines of Code (Simple Query) | 25–40 lines | 4–8 lines |
| Common Bugs ที่ ADO.NET เจอ | `IndexOutOfRange`, `NullRef`, `InvalidCast`, Ordinal Fragility, Resource Leak | Dapper handle ให้ทั้งหมด |
| Maintenance Cost | สูง (Schema เปลี่ยน = แก้ mapping ทุกที่) | ต่ำ (auto mapping) |

### 12.5 กรณีที่ต้องใช้ ADO.NET จริงๆ (Dapper ทำไม่ได้)

| Scenario | ทำไม Dapper ไม่ได้ | มีใน OmniChannel/RL? |
|---|---|---|
| `SqlBulkCopy` (Insert ล้านแถว) | ไม่มี built-in | ❌ ไม่มี |
| Streaming DataReader (พันล้านแถว) | Dapper materialize เป็น List | ❌ ไม่มี |
| TVP (Table-Valued Parameters) | Support จำกัด | ❌ ไม่มี |
| MARS | ไม่ expose control | ❌ ไม่มี |

### 12.6 แนวโน้ม Gap ใน .NET รุ่นใหม่

| .NET Version | Gap ADO.NET vs Dapper |
|---|---|
| .NET Framework | ~10%+ |
| .NET Core 3.1 | ~8-10% |
| .NET 8 | ~5% หรือน้อยกว่า |
| .NET 9/10 | **≤5%** (บาง benchmark Dapper เท่ากัน/เร็วกว่า) |

> Gap กำลัง **หดลงเรื่อยๆ** เพราะ runtime improvements (JIT PGO, Span\<T\>, async I/O) ช่วยทั้งคู่เท่าๆ กัน

### 12.7 Real-World Evidence

**Stack Overflow** (ผู้สร้าง Dapper):
- รองรับ **~2-3 พันล้าน page views/เดือน**
- ใช้ Dapper เป็น data layer หลัก
- **ไม่เคยต้อง fall back ไป raw ADO.NET** เพื่อ performance

### 12.8 🏆 สรุปการตัดสินใจ

| คำถาม | คำตอบ |
|---|---|
| ADO.NET เร็วกว่า Dapper แค่ไหน? | **≤5%** ใน .NET 8/9/10 (34μs ต่อ query) |
| คุ้มไหมที่จะสลับไป ADO.NET? | **❌ ไม่คุ้ม** — Code เพิ่ม 5-10x, Bug เพิ่ม, Maintenance สูงขึ้น |
| ใช้อะไรเป็นมาตรฐาน? | **✅ Dapper เป็นตัวหลักสำหรับทุกงาน** |
| เมื่อไหร่ถึงใช้ ADO.NET? | เฉพาะ `SqlBulkCopy` หรือ streaming เท่านั้น (ซึ่งไม่มีใน project เรา) |

> 📌 **มาตรฐานที่กำหนด:** ใช้ Dapper เป็น Data Access หลัก ทุกโปรเจค  
> ดูรายละเอียด Coding Standard ที่ [Coding_Standard_Criteria.md](../Coding_Standard_Criteria.md)

---

*เอกสารนี้จัดทำเพื่อประกอบการตัดสินใจ Data Access Strategy สำหรับโปรเจค OmniChannel*  
*วันที่: 4 มีนาคม 2026*  
*อัปเดตล่าสุด: 4 มีนาคม 2026 — เพิ่มผลวิจัย ADO.NET vs Dapper*
